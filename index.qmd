...
**Abstract.** We test whether trading-volume information predicts the cross-section of momentum returns, and whether any such effect depends on the timeframe at which it is measured. The study proceeds in three phases, each provoked by the failure of the last. **Phase 1** asks whether high-turnover ("crowded") momentum stocks in the S&P 500 subsequently underperform; they do not — high-turnover winners outperform at every horizon, contradicting the crowding-reversal hypothesis. **Phase 2** refines the signal from unsigned turnover to price-signed volume (on-balance volume) to separate accumulation from distribution, but returns a null result driven by a sample — a single equity bull market — in which genuine price/flow divergence almost never occurs (2.7% of observations). **Phase 3** moves to a multi-regime cryptocurrency panel (60 persistently-liquid Binance USDT pairs, 2021–2026), replaces the OBV proxy with true taker-flow CVD, and treats timeframe as the object of study — sweeping the identical test across monthly, weekly, daily, 4-hour, and 1-hour bars. Flow-confirmed momentum outperforms flow-divergent momentum at the weekly and daily scale (t up to 3.9) and is statistically indistinguishable from zero intraday. We read this as suggestive rather than conclusive: the largest test statistics arise at overlapping horizons that inflate significance, and the cleanest non-overlapping estimates fall just short of conventional thresholds. The contribution is as much methodological as empirical — a worked example of iterative hypothesis refinement, disciplined treatment of null results, and the data-engineering reality of point-in-time cross-sectional research.
:::
...
- **Phase 3 — CVD net taker flow (signed by aggressor).** The same confirmed/divergent split, but the signal is real net taker flow, `(taker buy − taker sell) / total volume`, bounded in [−1, 1].
...
::: {.callout-note title="Why Phase 2 happened"}
...
::: {.callout-note title="Why Phase 3 happened"}
...
On the multi-regime crypto panel with real CVD, divergence is no longer scarce: divergence rates run from 44% to 78% across the ladder, an order of magnitude above the equity sample, confirming the test finally has something to measure. The result (Table 3, @fig-significance) is a clear timeframe gradient. Confirmed momentum significantly outperforms divergent momentum at the *weekly* and *daily* scale — the spread strengthens with horizon and the t-statistic reaches 3.86 at weekly horizon 4 — while at 4-hour and 1-hour the spread collapses to essentially zero, with t-statistics near the origin. At the monthly scale the sign flips weakly negative but never approaches significance.
...
#| fig-cap: "t-statistic of the confirmed − divergent spread across the timeframe ladder. Each line traces one forward horizon; the shaded band is the non-significant region |t| < 1.96. The signal peaks at weekly/daily and decays to zero intraday — the paper's central finding. The cleanest series, horizon 1, peaks at t = 1.59 (weekly), just short of the threshold."
...
::: {.callout-note title="What the three phases actually taught me"}
...
::: {.callout-warning title="War story: the firewall"}
...
::: {.callout-warning title="War story: the LUNA blowup"}
...
::: {.callout-warning title="War story: the momentum-definition bug"}
...
4. Lee, C. M. C., & Swaminathan, B. (2000). Price Momentum and Trading Volume. *The Journal of Finance*, 55(5), 2017–2069.
